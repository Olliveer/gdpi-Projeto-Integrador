        ...
        try {
            ...
            for (FileItem item : items) {
                if (!item.isFormField()) {
                    ...
                    doc = new Documento(path, nomeDoCampo, Integer.valueOf(valorDoCampo), true);
                    processUploadedFile(item);
                }
            }
            
            request.getRequestDispatcher("Cadastrar_Doc?nome="+doc.getNome()+"&url="+doc.getUrl()+"&id_cli="+doc.getCliente_id()+"&adm="+doc.isAdm()+"&env=0").forward(request, response);
